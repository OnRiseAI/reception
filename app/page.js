...
// ─── Icons (same set, slightly larger) ───
const ICONS = {
  scale: (
  ...
  return (
    <>
      ...
      <div className="text-center mb-12 md:mb-16">
        ...
        <p className="mt-3 text-[16px] text-zinc-400 font-normal">
          Pick your industry. Talk to your AI receptionist. Free, right now.
        </p>
      </div>

      {/* Industry grid — big clear targets */}
        ...
        {VERTICAL_KEYS.map((key) => {
          ...
          return (
            <Link
              key={key}
              href={`/${key}`}
              className="group flex flex-col gap-3 p-5 rounded-2xl border border-zinc-800/80 bg-zinc-900/50 hover:bg-zinc-800/50 hover:border-zinc-700 transition-all duration-200 active:scale-[0.98]"
            >
              <div className="w-10 h-10 rounded-xl bg-[#10B981]/10 flex items-center justify-center text-zinc-500 group-hover:text-[#10B981] transition-colors">
                {ICONS[v.icon] || ICONS.building}
              </div>
              <div>
                <span className="block text-[15px] font-semibold text-zinc-200 group-hover:text-white transition-colors">
                  {v.label}
                </span>
                <span className="block text-[12px] text-zinc-600 mt-0.5 leading-relaxed">
                  Try it live →
                </span>
              </div>
            </Link>
